...
with
     __dbt__cte__aux_matriz_integracao_modo as (
...
with
    reparticao_unnest as (
        ...
        from `rj-smtr`.`planejamento`.`matriz_reparticao_tarifaria`, unnest(sequencia_modo) as modo
    ),
...
where modo_destino is not null
    ...
    transferencias as (
        select
            ...
            true as indicador_integracao
        from `rj-smtr-staging`.`source_smtr`.`matriz_transferencia`
    ),
    excecoes as (
        select
            date(trim(data_inicio)) as data_inicio,
        ...
        from `rj-smtr-staging`.`source_smtr`.`matriz_integracao_excecao`
    ),
    integracoes_regulares as (
        ...
        from __dbt__cte__aux_matriz_integracao_modo
    ),
    matriz_completa as (
        select *
