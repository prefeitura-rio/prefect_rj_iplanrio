...
with __dbt__cte__aux_veiculo_dia_consolidada as (
...
),  __dbt__cte__viagem_transacao_aux as (
        ...
        when
            ...
            and (
                ...
                or (
                    v.data >= date("2024-11-16")
                    and (
                        (coalesce(tr.quantidade_transacao_riocard, 0) = 0  and coalesce(t.quantidade_transacao, 0) = 0)
                        or coalesce(eev.indicador_estado_equipamento_aberto, false)
                        = false
                    )
                )
                or (
                    v.data >= date('2025-07-01')
                    and (coalesce(tr.quantidade_transacao_riocard, 0) = 0  and coalesce(t.quantidade_transacao, 0) = 0)
                )
            )
