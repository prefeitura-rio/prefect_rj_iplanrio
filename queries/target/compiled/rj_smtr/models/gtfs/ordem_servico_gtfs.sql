    ...
    with
        ordem_servico as (
            select
                ...
                safe_cast(
                    json_value(os.content, '$.horario_fim') as string
                ) horario_fim,
                safe_cast(
                    json_value(os.content, '$.extensao_ida') as float64
                ) extensao_ida,
                safe_cast(
            ...
            from `rj-smtr-staging`.`br_rj_riodejaneiro_gtfs_staging`.`ordem_servico` os
            join
                `rj-smtr`.`gtfs`.`feed_info` fi
                on os.data_versao = cast(fi.feed_start_date as string)
            where
                    os.data_versao = '2024-12-31'
                    and fi.feed_start_date = '2024-12-31'
        )
    select *, '' as versao_modelo
    from
