        ...
        dates as (
            select
                ...
                case
                    ...
                    then date(2022, 6, 1)
                        ...
                        when
                            data
                            between date(2022,7, 1) and date(2022,7, 15)
                        then date(2022,7, 1)
                        when
                            data between date(2022,7, 16) and last_day(
                                date(2022,7, 30), month
                            )
                        then date(2022,7, 16)
                    
                        when
                            data
                            between date(2022,8, 1) and date(2022,8, 15)
                        then date(2022,8, 1)
                        when
                            data between date(2022,8, 16) and last_day(
                                date(2022,8, 30), month
                            )
                        then date(2022,8, 16)
                    
                        when
                            data
                            between date(2022,9, 1) and date(2022,9, 15)
                        then date(2022,9, 1)
                        when
                            data between date(2022,9, 16) and last_day(
                                date(2022,9, 30), month
                            )
                        then date(2022,9, 16)
                    
                        when
                            data
                            between date(2022,10, 1) and date(2022,10, 15)
                        then date(2022,10, 1)
                        when
                            data between date(2022,10, 16) and last_day(
                                date(2022,10, 30), month
                            )
                        then date(2022,10, 16)
                    
                        when
                            data
                            between date(2022,11, 1) and date(2022,11, 15)
                        then date(2022,11, 1)
                        when
                            data between date(2022,11, 16) and last_day(
                                date(2022,11, 30), month
                            )
                        then date(2022,11, 16)
                    
                        when
                            data
                            between date(2022,12, 1) and date(2022,12, 15)
                        then date(2022,12, 1)
                        when
                            data between date(2022,12, 16) and last_day(
                                date(2022,12, 30), month
                            )
                        then date(2022,12, 16)
                    ...
                    -- 2023 a 2024:
                            ...
                            when
                                extract(month from data) = 4
                                and extract(year from data) = 2023
                            then date(2023,4, 1)
                        
                            when
                                extract(month from data) = 5
                                and extract(year from data) = 2023
                            then date(2023,5, 1)
                        
                            when
                                extract(month from data) = 6
                                and extract(year from data) = 2023
                            then date(2023,6, 1)
                ...
                case
                    ...
                    -- 2022:
                        ...
                        when
                            data
                            between date(2022,6, 1) and date(2022,6, 15)
                        then date(2022,6, 1)
                        when
                            data between date(2022,6, 16) and last_day(
                                date(2022,6, 30), month
                            )
                        then date(2022,6, 16)
            ...
            from
                unnest(
                    generate_date_array(
                        "2022-06-01",
                        date_sub("2024-04-01", interval 1 day)
                    )
                ) as data
        ),
        trips as (
            ...
            from `rj-smtr`.`projeto_subsidio_sppo`.`subsidio_trips_desaninhada`
                ...
                where
                    data_versao >= date_trunc(
                        date_sub(date("2022-01-01T01:00:00"), interval 30 day), month
                    )
            
        ),
        shapes as (
            select distinct data_versao
            from `rj-smtr-staging.projeto_subsidio_sppo_staging.shapes`
