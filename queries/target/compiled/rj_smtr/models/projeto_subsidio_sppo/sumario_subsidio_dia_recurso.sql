with sumario as (
  ...
  FROM `rj-smtr`.`projeto_subsidio_sppo`.`sumario_subsidio_dia_periodo_recurso`
  group by 1,2,3,4
),
valor as (
  select
  ...
  from
    sumario s
  left join
    `rj-smtr.projeto_subsidio_sppo.subsidio_data_versao_efetiva` v -- `rj-smtr`.`projeto_subsidio_sppo`.`subsidio_data_versao_efetiva` v
  on v.data = s.data
)
select 
  *,
