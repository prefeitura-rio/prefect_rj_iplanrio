...
with viagem_paga as (
    ...
    from 
        `rj-smtr.projeto_subsidio_sppo.viagem_completa` -- `rj-smtr`.`projeto_subsidio_sppo`.`viagem_completa` -- todo: ref to prod
    group by
        1,2
),
viagem_recurso as (
    select
        ...
        count(id_viagem) as viagens_realizadas
    from 
        `rj-smtr`.`projeto_subsidio_sppo`.`viagem_completa_recurso`
    group by
        1,2
),
viagem as (
...
planejado as (
    ...
    from (
        ...
        from
            `rj-smtr.projeto_subsidio_sppo.viagem_planejada` -- `rj-smtr`.`projeto_subsidio_sppo`.`viagem_planejada` # todo: ref to prod
