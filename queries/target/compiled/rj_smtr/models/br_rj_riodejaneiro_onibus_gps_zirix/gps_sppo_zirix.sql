...
WITH
  ...
  intersec AS (
    ...
    velocidades as (
        ...
        from __dbt__cte__sppo_aux_registros_velocidade_zirix
    ),
    paradas as (
        -- 3. paradas
    ...
    flags as (
        ...
        from __dbt__cte__sppo_aux_registros_flag_trajeto_correto_zirix
    )
-- 5. Junção final
select
    "SPPO" modo,
...
join
    ...
    and r.linha = p.linha
where
        date(r.timestamp_gps) between date("2022-01-01T00:00:00") and date(
            "2022-01-01T01:00:00"
        )
        and r.timestamp_gps > "2022-01-01T00:00:00"
