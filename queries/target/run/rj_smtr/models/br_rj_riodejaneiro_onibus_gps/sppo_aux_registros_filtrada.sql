    ...
    merge into `rj-smtr`.`br_rj_riodejaneiro_onibus_gps`.`sppo_aux_registros_filtrada` as DBT_INTERNAL_DEST
        using (
...
WHERE
  rn = 1
    ...
    when not matched then insert
        (`id_veiculo`, `latitude`, `longitude`, `posicao_veiculo_geo`, `velocidade`, `linha`, `timestamp_gps`, `timestamp_captura`, `data`, `hora`, `versao`)
    values
        (`id_veiculo`, `latitude`, `longitude`, `posicao_veiculo_geo`, `velocidade`, `linha`, `timestamp_gps`, `timestamp_captura`, `data`, `hora`, `versao`)
