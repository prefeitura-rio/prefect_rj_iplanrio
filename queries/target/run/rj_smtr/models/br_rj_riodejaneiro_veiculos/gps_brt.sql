-- back compat for old kwarg name
    ...
    merge into `rj-smtr-dev`.`br_rj_riodejaneiro_veiculos`.`gps_brt` as DBT_INTERNAL_DEST
        using (
/*
Descrição:
...
with 
    ...
    FROM  `rj-smtr-dev`.`br_rj_riodejaneiro_brt_gps`.`brt_aux_registros_filtrada`
    WHERE
...
WITH 
  ...
  distancia AS (
    ...
    JOIN (
      SELECT *
      FROM
        `rj-smtr-dev`.`br_rj_riodejaneiro_brt_gps`.`brt_aux_registros_filtrada`
      WHERE
      data between DATE("2023-12-14T08:00:00") and DATE("2024-04-25T12:00:00")
      AND timestamp_gps > "2023-12-14T08:00:00" and timestamp_gps <="2024-04-25T12:00:00"
...
WITH
  registros AS (
    ...
    FROM
      `rj-smtr-dev`.`br_rj_riodejaneiro_brt_gps`.`brt_aux_registros_filtrada` r
    WHERE
    data between DATE("2023-12-14T08:00:00") and DATE("2024-04-25T12:00:00")
    AND timestamp_gps > "2023-12-14T08:00:00" and timestamp_gps <="2024-04-25T12:00:00"
  ),
...
WHERE
  date(r.timestamp_gps) between DATE("2023-12-14T08:00:00") and DATE("2024-04-25T12:00:00")
  AND r.timestamp_gps > "2023-12-14T08:00:00" and r.timestamp_gps <="2024-04-25T12:00:00"
