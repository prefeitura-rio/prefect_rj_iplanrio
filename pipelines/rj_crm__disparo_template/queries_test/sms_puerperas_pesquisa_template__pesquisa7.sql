...
-- janela D+40 e simulam que o CPF já recebeu a primeira HSM (D0, smspuerperasdisparo25),
-- pré-requisito do CTE filtra_recebeu_primeira_hsm; passo 3 é a query em si.
...
(
    '12345678901',
    'MARIA SALESFORCE',
    -- pesquisa 7 dispara em D+40
    DATE_SUB(CURRENT_DATE('America/Sao_Paulo'), INTERVAL 40 DAY),
    '1234567',
...
-- ===================== 3) QUERY (query original da pesquisa 7) =====================
WITH segmentacao_original AS (
    ...
    AND CURRENT_DATE('America/Sao_Paulo') in
        (
        DATE_ADD(DATE(data_alta_internacao), INTERVAL 40 DAY)
        )
...
filtra_disparados as (
    ...
    -- tanto via status_disparo quanto via wetalkie (histórico legado pré-migração;
    -- templateId 589 = HSM da Pesquisa 7)
