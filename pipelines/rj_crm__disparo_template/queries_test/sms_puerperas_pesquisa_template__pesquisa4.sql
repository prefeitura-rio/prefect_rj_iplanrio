...
-- janela D+7/D+22 e simulam que o CPF já recebeu a primeira HSM (D0,
...
-- ===================== 3) QUERY (query original da pesquisa 4) =====================
WITH segmentacao_original AS (
    ...
    AND CURRENT_DATE('America/Sao_Paulo') in
        (
        DATE_ADD(DATE(data_alta_internacao), INTERVAL 7 DAY),
        DATE_ADD(DATE(data_alta_internacao), INTERVAL 22 DAY)
        )
...
filtra_disparados as (
    ...
    -- tanto via status_disparo quanto via wetalkie (histórico legado pré-migração; essa
    -- campanha unifica os antigos disparos D7/Pesquisa2 (templateId 456) e
    -- D22/Pesquisa4 (templateId 489), então checamos os dois)
    select filtra_recebeu_primeira_hsm.*
    FROM filtra_recebeu_primeira_hsm
    left join `rj-crm-registry.brutos_salesforce.status_disparo` sd
            on sd.cpf = filtra_recebeu_primeira_hsm.cpf
            and sd.nome_hsm = 'smspuerperadisparo6'
