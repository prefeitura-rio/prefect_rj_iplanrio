-- Cartão PIC - Aviso de Evento (smascartaoprimeirainfanciaprodv27)
-- Query de teste: lê de rj-crm-registry-dev.teste.cartao_primeira_infancia_carioca_status_teste
-- DE esperada: whatsapp_smas_cartaopic_evento_<timestamp>
...
VALUES (
  ...
  DATE_ADD(CURRENT_DATE('America/Sao_Paulo'), INTERVAL 5 DAY),
  '10:00',
  'CRAS Vila Isabel',
  'Rua Torres Homem, 120 - Vila Isabel',
  DATE_ADD(CURRENT_DATE('America/Sao_Paulo'), INTERVAL 5 DAY),
    ...
    SELECT
        ...
        TRIM(
            CONCAT(
                LOCAL_ENTREGA_PREVISTO,
                IF(
                    ENDERECO_ENTREGA_PREVISTO IS NOT NULL AND ENDERECO_ENTREGA_PREVISTO != '',
                    CONCAT(' (', ENDERECO_ENTREGA_PREVISTO, ')'),
                    ''
                )
            )
        ) AS endereco_evento,
        DATA_ENTREGA_PREVISTA AS data_evento,
        HORA_ENTREGA_PREVISTA AS horario_evento,
