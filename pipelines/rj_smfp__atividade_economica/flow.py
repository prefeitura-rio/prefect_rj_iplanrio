# -*- coding: utf-8 -*-
"""
Flow unificado para dump de tabelas do banco de dados de Atividade Econômica - SMFP.

Este flow é responsável por extrair dados do banco SQL Server DW_BI_ALVARAS
e carregar no BigQuery. Processa qualquer tabela baseado no parâmetro table_id.
...
from pipelines.rj_smfp__atividade_economica.constants import (
    TABLE_CONFIGS,
    Constants,
)
...
def rj_smfp__atividade_economica(
    table_id: str,
    ...
    """
    Flow unificado para dump de tabelas de Atividade Econômica do SMFP.

    Este flow extrai dados do banco SQL Server DW_BI_ALVARAS (Alvarás)
    e carrega no BigQuery. Processa qualquer tabela configurada em TABLE_CONFIGS
    baseado no parâmetro table_id.

    Args:
        table_id: Nome da tabela a ser processada (ex: 'fact_fatoalvaras', 'tab_alvara')
    ...
    Examples:
        Para processar a tabela fact_fatoalvaras:
        >>> rj_smfp__atividade_economica(table_id="fact_fatoalvaras")

        Para processar tab_alvara com modo específico:
        >>> rj_smfp__atividade_economica(table_id="tab_alvara", dump_mode="append")
    """
    # Renomear o flow run para facilitar identificação
    rename_current_flow_run_task(new_name=f"atividade_economica_{table_id}")
