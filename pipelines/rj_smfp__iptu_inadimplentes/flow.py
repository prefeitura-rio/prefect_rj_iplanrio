# -*- coding: utf-8 -*-
"""
Flow unificado para dump de tabelas do banco de dados de IPTU Inadimplentes - SMFP.

Este flow é responsável por extrair dados do banco SQL Server DBINAD
(IPTU Inadimplentes) e carregar no BigQuery. Processa qualquer tabela
...
from pipelines.rj_smfp__iptu_inadimplentes.constants import (
    TABLE_CONFIGS,
    Constants,
)
...
def rj_smfp__iptu_inadimplentes(
    table_id: str,
    ...
    """
    Flow unificado para dump de tabelas de IPTU Inadimplentes da SMFP.

    Este flow extrai dados do banco SQL Server DBINAD (Base de Dados de IPTU Inadimplentes)
    contendo informações sobre contribuintes inadimplentes do IPTU (Imposto Predial e
    Territorial Urbano) no município do Rio de Janeiro, carregando-os no BigQuery.
    Processa qualquer tabela configurada em TABLE_CONFIGS baseado no parâmetro table_id.

    As tabelas contêm informações sobre:
    - Perfil de contribuintes inadimplentes do IPTU
    - Histórico de inadimplência
    - Dados cadastrais dos imóveis
    - Valores devidos e situação fiscal

    Args:
        table_id: Nome da tabela a ser processada (ex: 'perfil_inadimplente_v2')
        db_database: Nome do banco de dados SQL Server (padrão: 'DBINAD')
        db_host: Host do banco de dados (padrão: '10.3.23.158')
        ...
        dataset_id: ID do dataset no BigQuery (padrão: 'iptu_inadimplentes')
    ...
    Examples:
        Para processar a tabela perfil_inadimplente_v2:
        >>> rj_smfp__iptu_inadimplentes(table_id="perfil_inadimplente_v2")

        Para processar com modo específico:
        >>> rj_smfp__iptu_inadimplentes(
        ...     table_id="perfil_inadimplente_v2",
        ...     dump_mode="overwrite"
        ... )

    Notes:
        - O flow utiliza modo 'overwrite' por padrão (substitui dados existentes)
        - As credenciais do banco são obtidas do Infisical
        - A tabela perfil_inadimplente_v2 contém todos os dados de IPTU inadimplentes
        - Dados são atualizados semanalmente conforme o schedule configurado
    """
    # Renomear o flow run para facilitar identificação
    rename_current_flow_run_task(new_name=f"iptu_inadimplentes_{table_id}")
