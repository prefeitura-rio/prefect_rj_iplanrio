...
def is_duplicate_nf(
    ...
    :param validator: The ``ComplianceValidator`` instance (supplies the deduplication lookup).
    ...
    pdf_list = validator.deduplication_lookup.get(dedup_key, [])
    ...
    if data_envio is None:
        data_envio = validator.pdf_data_envio.get(pdf_name)
...
def handle_single_match(
    ...
    :param validator: The ``ComplianceValidator`` instance.
...
def handle_multiple_matches(
    ...
    return handle_single_match(validator, expected_nf, best_extracted, cnpj_start_dates, page_categories)


def handle_nf_ticket_merge(
    validator: "ComplianceValidator",
    expected_nf: dict,
    nf: dict,
    ticket: dict,
    ...
    result = handle_single_match(validator, expected_nf, merged, cnpj_start_dates, page_categories)
...
def process_single_declaration(
    validator: "ComplianceValidator",
    expected_nf: dict,
    extracted_nfs: list[dict],
    cnpj_start_dates: dict,
    ...
    if nf and ticket:
        ...
        return handle_nf_ticket_merge(validator, expected_nf, nf, ticket, match_type, cnpj_start_dates, page_categories)
    ...
    perfect_matches = find_standard_matches(
        expected_nf["cnpj_norm"],
        expected_nf["numero_norm"],
    ...
    if len(perfect_matches) == 0 and validator.min_match_score < 3:
        standard_matches = find_standard_matches(
            expected_nf["cnpj_norm"],
            expected_nf["numero_norm"],
            expected_nf["original"].get("data_emissao"),
            extracted_nfs,
            min_match_score=validator.min_match_score,  # Allow partial match per config
        )
    else:
    ...
    if len(standard_matches) == 0:
        # No match
        return handle_missing_nf(validator, expected_nf, page_categories)

    elif len(standard_matches) == 1:
        # Case A: Single match
        return handle_single_match(validator, expected_nf, standard_matches[0], cnpj_start_dates, page_categories)

    else:
        # Case B: Multiple matches (prioritization)
        return handle_multiple_matches(validator, expected_nf, standard_matches, cnpj_start_dates, page_categories)
