...
def print_validation_report(validation: dict, verbose: bool = True) -> None:
    """
        ...
        if validation["suspicious_extractions"]:
            ...
            for item in validation["suspicious_extractions"]:
                ...
                logger.info(
                    ...
                    ext.get("cnpj_emitente", "N/A"),
                    ext.get("numero_nf", "N/A"),
                    item["reason"],
                )
        ...
        if validation["normalization_issues"]:
            ...
            for item in validation["normalization_issues"]:
                ...
                logger.info(
                    ...
                    exp["numero_nf"],
                    item["extracted_numero"],
                    item["issue"],
                )


def print_batch_report(batch_validation: dict, group_by_nf: bool = True) -> None:
    """
    ...
    if group_by_nf:
        _print_nf_centric_report(batch_validation)


def _print_nf_centric_report(batch_validation: dict) -> None:
    """
    ...
    for nf in nf_statuses:
        if nf["status"] == "FOUND":
            ...
            missing_count += 1
            logger.warning(
                "[MISS] NOT FOUND | PDF: %s | CNPJ: %s | Numero: %s | Valor: %s | Classificação: %s | Página: %s",
                nf["pdf_name"],
                nf["cnpj"],
                nf["numero_nf"],
                nf["valor_total"],
