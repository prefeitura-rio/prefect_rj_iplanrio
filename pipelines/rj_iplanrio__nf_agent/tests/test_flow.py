...
def test_submit_action_forwards_parameters():
    with (
        ...
        run_flow(acao="submeter", origem="gs://b/p", max_paginas=10, versao_processamento="x")
    poll_task.assert_not_called()
    kwargs = submit_task.call_args.kwargs
    assert (kwargs["origem"], kwargs["max_paginas"], kwargs["versao_processamento"]) == ("gs://b/p", 10, "x")
