# -*- coding: utf-8 -*-
"""
Flow unificado para dump de tabelas do banco de dados de Instrumentos Firmados - SMFP.

Este flow é responsável por extrair dados do banco SQL Server DW_IGAA
(FINCON - Gestão de Instrumentos Jurídicos) e carregar no BigQuery.
Processa qualquer tabela baseado no parâmetro table_id.
...
from pipelines.rj_smfp__adm_instrumentos_firmados.constants import (
    TABLE_CONFIGS,
    Constants,
)
...
def rj_smfp__adm_instrumentos_firmados(  # noqa: PLR0913
    table_id: str,
    ...
    """
    Flow unificado para dump de tabelas de Instrumentos Firmados (FINCON) da SMFP.

    Este flow extrai dados do banco SQL Server DW_IGAA (Data Warehouse do IGAA -
    Instituto de Gestão de Águas e Alagamentos), especificamente da view
    VINSTRUMENTOS_JURIDICOS_ESCRITORIO_GBP, que contém informações sobre instrumentos
    jurídicos firmados pela administração municipal (contratos, convênios, termos de
    cooperação, etc.), carregando-os no BigQuery.

    Processa qualquer tabela configurada em TABLE_CONFIGS baseado no parâmetro table_id.

    As tabelas contêm informações sobre:
    - Instrumentos jurídicos (contratos, convênios, termos)
    - Dados do favorecido (CNPJ, nome)
    - Valores (empenhado, liquidado, pago, saldo)
    - Datas (assinatura, início, fim)
    - Órgão executor e modalidade de licitação
    - Situação e espécie do instrumento

    Args:
        table_id: Nome da tabela a ser processada (ex: 'instrumento_firmado')
        db_database: Nome do banco de dados SQL Server (padrão: 'DW_IGAA')
        db_host: Host do banco de dados (padrão: '10.2.221.17')
        ...
        dataset_id: ID do dataset no BigQuery (padrão: 'adm_instrumentos_firmados')
    ...
    Examples:
        Para processar a tabela instrumento_firmado:
        >>> rj_smfp__adm_instrumentos_firmados(table_id="instrumento_firmado")

        Para processar com modo específico:
        >>> rj_smfp__adm_instrumentos_firmados(
        ...     table_id="instrumento_firmado",
        ...     dump_mode="overwrite"
        ... )

    Notes:
        - O flow utiliza modo 'overwrite' por padrão (substitui dados completos diariamente)
        - As credenciais do banco são obtidas do Infisical
        - A view VINSTRUMENTOS_JURIDICOS_ESCRITORIO_GBP é uma view do Escritório GBP
        - Dados são atualizados diariamente conforme o schedule configurado
        - A tabela contém 24 campos relacionados a instrumentos jurídicos
    """
    # Renomear o flow run para facilitar identificação
    rename_current_flow_run_task(new_name=f"adm_instrumentos_firmados_{table_id}")
